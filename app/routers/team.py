...
async def get_full_info_teams():
    return await service.get_full_info_teams()
...
async def get_free_teams():
    return await service.get_free_teams()


@router.get(path="/by_userId/{user_id}",
            summary="Получить бригаду работника",
            response_model=TeamModelSchema)
async def get_team_by_user_id(user_id: int):
    return await service.get_team_by_user_id(user_id)
...
@router.post(path="/",
             ...
             response_model=TeamModelSchema,
             status_code=201)
async def create_team(new_team: TeamCreateSchema):
    return await service.add_team(new_team)
...
@router.patch(path="/move",
              ...
              response_model=TeamModelSchema)
async def move_team(team_id: int, new_coordinates: CoordinatesSchema):
    return await service.move_team(team_id, new_coordinates)
